...
def drawShortesPath(cameFrom,curentCell,drawGrid):
    while curentCell in cameFrom:
        curentCell=cameFrom[curentCell]
        curentCell.color=PURPLE
        drawGrid()

def algorithmA(drawGrid,grid,start,end):
    pq=PriorityQueue()
    tieBreaker=0
    pq.put((0,tieBreaker,start))
    cameFrom={}
    ...
    while not pq.empty():
        ...
        if curentCell==end:
            ...
            return True
        for ns in curentCell.nbrs:
            tempG=g[curentCell]+1
            if tempG<g[ns]:
                cameFrom[ns]=curentCell
                g[ns]=tempG
                f[ns]=tempG+hureisticValue(ns.getPos(), end.getPos())
                if ns not in openSet:
                    tieBreaker+=1
                    pq.put((f[ns],tieBreaker,ns))
                    openSet.add(ns)
                    # ns.color=GREEN
        drawGrid()
        if curentCell!=start:
